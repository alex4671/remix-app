			...
			if (userToInvite) {
				if (collaboratorsIds.includes(userToInvite.id)) {
					return json({
						success: false,
						intent,
						message: `Collaborator already invited`,
					});
				}
				...
				return json({
					success: true,
					intent,
					message: `Collaborator ${userToInvite.email} added`,
				});
			}
	...
	if (intent === 'removeAccess') {
		...
		try {
			...
			emitter.emit(
				...
				[user.id, deletedCollaborator.user.id],
				sessionId,
			);
			return json({
				success: true,
				intent,
				message: `${deletedCollaborator.user.email} removed`,
			});
		} catch (e) {
			return json({
				success: false,
	...
	if (intent === 'updateRights') {
		...
		try {
			...
			return json({
				success: true,
				intent,
				message: `${updatedCollaborator.user.email} rights updated`,
			});
		} catch (e) {
	...
	return (
		...
		>
			...
			>
				...
				<Badge color={isUserOwner ? 'emerald' : 'yellow'}>
					Owner ({isUserOwner ? 'You' : workspace?.owner.email})
				</Badge>
			</Group>
