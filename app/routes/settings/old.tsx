...
export const action = async ({ request }: ActionArgs) => {
	...
	if (intent === 'pause') {
		try {
			...
			await paddle.updateUser({
				subscription_id: Number(subscriptionId),
				pause: !userSubscription?.[0].paused_at,
			});
			return json({ success: true, intent });
		} catch (e) {
			return json({ success: false, intent });
	...
	if (intent === 'addModifier') {
		...
		const amount =
			modifierType === 'remove' ? `-${modifierAmount}` : modifierAmount;
...
export default function Pro() {
	...
	const { transactions, modifiers, userSubscription } =
