...
export const loader = async ({request}: LoaderArgs) => {
  ...
  if (user.payment) {
    ...
    const userSubscription = await paddle.listUsers({subscription_id: Number(subscriptionId)})
...
export const action = async ({request}: ActionArgs) => {
  ...
  if (intent === "pause") {
    ...
    try {
      const userSubscription = await paddle.listUsers({subscription_id: Number(subscriptionId)})
      // @ts-ignore
      await paddle.updateUser({subscription_id: Number(subscriptionId), pause: !userSubscription?.[0].paused_at})
      return json({success: true, intent})
