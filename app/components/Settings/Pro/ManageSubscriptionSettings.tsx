...
export const ManageSubscriptionSettings = () => {
	...
	const [selectedPlan, setSelectedPlan] = useState<string | null>(
		userSubscription.plan_id === 26607 ? '26608' : '26607',
	);
	...
	const checkoutCancelComplete = (data: any) => {
		...
		if (data.checkout.completed) {
			navigate('/payment/unsubscribed', {
				state: dayjs(payment?.subscriptionEndDate).format('MMMM D, YYYY'),
			});
		}
	};
				...
				<Box p={'lg'}>
					<Group spacing={6}>
						...
						<Tooltip
							...
							label={
								<Text>
									Next payment{' '}
									{formatMoney(
										userSubscription?.next_payment?.amount,
										userSubscription?.next_payment?.currency,
									)}{' '}
									{dayjs(userSubscription?.next_payment?.date).format(
										'MMMM D, YYYY',
									)}
								</Text>
							}
						...
						>
							<Badge
								color={'emerald'}
								sx={{ cursor: isSubscriptionDeleted ? 'default' : 'pointer' }}
								rightSection={
							...
							>
								Pro ({plans[String(userSubscription.plan_id)]})
							</Badge>
						</Tooltip>
					</Group>
					...
					<Box my={12}>
						{isSubscriptionDeleted ? (
							<Text>
								...
								<Badge color={'emerald'}>
									{dayjs(payment?.subscriptionEndDate).format('MMMM D, YYYY')}
								</Badge>
							</Text>
						) : (
										...
										data={[
											{
												label: 'Daily',
												value: '26607',
												disabled: userSubscription.plan_id === 26607,
											},
											{
												label: 'Monthly',
												value: '26608',
												disabled: userSubscription.plan_id === 26608,
											},
											{
												label: 'Yearly',
												value: '26609',
					...
					<Box py={12}>
						{userSubscription.state === 'deleted' ? (
							<Text>
								You can subscribe again when current subscription expires
							</Text>
						) : (
							<Group
								mt={12}
								position={'apart'}
							>
								<PrimaryButton onClick={handleUpdateSubscription}>
